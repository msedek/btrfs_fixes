/*
 * fix_uuid_tree.c — Create an empty UUID tree and update its ROOT_ITEM
 *
 * The UUID_TREE ROOT_ITEM points to a block that now belongs to the EXTENT_TREE.
 * This program creates a new empty leaf for the UUID tree and updates the ROOT_ITEM.
 * The kernel regenerates UUID tree contents automatically on RW mount.
 *
 * Build: make fix_uuid_tree (add target to Makefile first)
 * Run: sudo ./fix_uuid_tree /dev/sdX
 */
...
int main(int argc, char *argv[])
{
	...
	leaf = btrfs_alloc_tree_block(trans, tree_root, 0, BTRFS_UUID_TREE_OBJECTID, NULL, 0, 0, 0, 0);
	if (IS_ERR(leaf)) {
	...
	btrfs_set_header_owner(leaf, BTRFS_UUID_TREE_OBJECTID);
	...
	/* Update UUID_TREE ROOT_ITEM in the root tree */
	printf("Updating UUID_TREE ROOT_ITEM...\n");
	...
	if (ret != 0) {
		fprintf(stderr, "ERROR: UUID_TREE ROOT_ITEM not found (ret=%d)\n", ret);
